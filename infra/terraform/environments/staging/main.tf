# Ambiente de workload Staging (ver ADR-0011). Topologia
# equivalente a Production (mesma estrutura de modulos), escala reduzida.
# Promove os MESMOS 4 digests ja validados por release-qualification -
...
module "secrets" {
  ...
  secrets = {
    ...
    # Credenciais dedicadas de MIGRACAO (papel de banco com privilegio de
    # DDL, distinto do papel runtime das APIs/Worker/Publisher - mesma
    # convencao ja usada localmente via docker-compose.yml, usuarios
    # "ledger_migration"/"consolidation_migration") - usadas exclusivamente
    # pela task ECS one-off do MigrationRunner, nunca pelos 4 workloads de negocio.
    "banco-carrefour/${local.environment}/ledger-migration/db-credentials"        = { description = "Credenciais de banco (papel de migracao/DDL) do MigrationRunner - fronteira Ledger (${local.environment})." }
    "banco-carrefour/${local.environment}/consolidation-migration/db-credentials" = { description = "Credenciais de banco (papel de migracao/DDL) do MigrationRunner - fronteira Consolidation (${local.environment})." }
  }
...
module "task_roles" {
  ...
  roles = {
    ...
    # exclusivas (
    # Closure, secao 6) - nunca uma unica role com acesso aos secrets de
    # AMBAS as fronteiras. A migracao Ledger nunca pode ler o secret de
...
module "task_execution_roles" {
  ...
  roles = {
    ...
    # Execution role generica e compartilhada pelas DUAS task roles de
    # migracao (Ledger e Consolidation) -, secao 6: "task
    # execution roles podem compartilhar apenas as permissoes genericas
    # minimas necessarias para puxar a imagem e escrever logs, quando
    # justificado". Nunca precisa de secret_arns: o MigrationRunner resolve
    # o secret de conexao ele mesmo, via SDK, usando a permissao da TASK
    # role (nunca da execution role) - a execution role so existe para
...
# --- Tasks ECS one-off de migracao (Release Boundary and Deployment Safety
# Closure, secoes 5-6) - NUNCA um aws_ecs_service, executadas
# exclusivamente via "aws ecs run-task". DUAS instancias, uma por
# fronteira - NUNCA uma unica task definition/role compartilhada entre
# Ledger e Consolidation (isolamento de IAM real e testavel). Nunca um 5o
# workload de negocio no modelo de dominio arquitetural.
...
# Regras simetricas de ingress (criadas FORA do modulo network para evitar
# dependencia circular - o mesmo padrao ja usado para o VPC Link V2 -> ALB,
# ver modulo edge): cada security group dedicado de task de migracao
# precisa de acesso ao RDS e aos VPC endpoints, mas o modulo network nao
# pode referenciar um security group criado por um modulo que so existe
# depois dele. RDS_security_group_id permanece um UNICO objeto
# compartilhado por Ledger e Consolidation neste modulo de rede (nao
# alterado por esta auditoria - fora do escopo de uma correcao restrita) -
# o isolamento real e testavel entre fronteiras de migracao e o de
# IAM/secret (task roles dedicadas), nunca dependente so deste SG
# compartilhado.
...
# --- IAM/OIDC: role de deploy assumida pelo workflow de promocao a Staging (ver ADR-0014) ---
