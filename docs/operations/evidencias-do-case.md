...
| Servico de consolidado diario | `Consolidation.Worker`, `Consolidation.Persistence`, `DailyBalance` e `ProcessedEvent`; documentos em `docs/architecture/08-implementation-readiness.md` e `docs/traceability.md`. | Implementado | Consolidado e visao derivada e eventualmente consistente; `DailyBalance` usa upsert atomico para evitar lost update no banco. |
...
| Consolidado suporta pico de 50 RPS com no maximo 5% de falha/perda | `docs/operations/teste-de-carga-consolidado.md`; `tests/Consolidation.LoadTests`. | Validado localmente/container-first | Evidencia observada: JWT com issuer/audience, 50.02 req/s sustentado, 0% falhas, p95 6.49 ms, p99 8.68 ms e throughput minimo de 50 RPS atendido. Nao substitui validacao produtiva. |
