...
/// <summary>
/// Compõe a connection string do Consolidation a partir de
/// <c>ConnectionStrings:Consolidation</c> (host/porta/database — não
/// sensível) + credenciais obtidas do Secrets Manager. O bypass
/// (<see cref="SecretsManagerOptions.Enabled"/> = false) é explícito e
/// restrito ao ambiente "Testing" (<see cref="EnvironmentGuard"/>,
/// ADR-0009/ADR-0009): quando desligado, <c>ConnectionStrings:Consolidation</c>
...
/// </summary>
public static class ConsolidationConnectionStringResolver
{
    public static string Resolve(IConfiguration configuration, string environmentName)
    {
        var baseConnectionString = configuration.GetConnectionString("Consolidation")
            ?? throw new InvalidOperationException("ConnectionStrings:Consolidation não configurado.");
