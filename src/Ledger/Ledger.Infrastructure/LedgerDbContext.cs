...
public sealed class LedgerDbContext(DbContextOptions<LedgerDbContext> options) : DbContext(options)
{
    ...
    private static void ConfigureOutboxMessages(ModelBuilder modelBuilder)
    {
        ...
        outboxMessage.HasIndex(x => new { x.Status, x.NextAttemptAt, x.CreatedAt });
        ...
        outboxMessage.Property(x => x.NextAttemptAt).HasColumnName("next_attempt_at");
        ...
        outboxMessage.Property(x => x.LockedBy).HasColumnName("locked_by").HasMaxLength(128);
    }
}
